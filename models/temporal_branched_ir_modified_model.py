...
class TemporalBranchedIRModifiedModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_GAN', type=float, default=1.0,
                                help='weight of GAN loss for generator and discriminator')
        return parser
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(self.device, opt.alter_initial_model, opt.unfreeze_iter, opt.initial_model_path,
                                      opt.n_input_samples, opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
        ...
        if self.isTrain and opt.lambda_GAN != 0:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            self.netD = networks.define_D(opt.n_input_samples * opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
        ...
        if opt.use_perceptual_loss:
            ...
            self.netL = util.LossNetwork(opt.vgg16_path, [8, 15, 22, 29], self.device)
        else:
            if self.isTrain and opt.lambda_GAN != 0:
